...
def get_safest_dir(pacman: Entity, dir_weights):
    ...
    for _dir in get_valid_directions(pacman):
        ...
        for k, v in dir_weights.items():
            # pygame.draw.line(pygame.display.get_surface(), (255, 0, 0), pacman.position.asTuple(), (pacman.position + k * v * 100).asTuple())
            # pygame.display.update()
            if k == _dir_vec:
                _weight += v
        if _weight < weight:
...
def dijkstras(pacman: Entity, ghost: Ghost):
    ...
    while distances[ghost.node] == math.inf and len(unvisited) > 0:
        # Following loop takes the node with the least distance and puts it in next_node
        next_node_i = 0
